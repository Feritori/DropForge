...
function createTables($pdo) {
    ...
    // ========================================================================
    // Читаем и выполняем database.sql
    // ========================================================================
    
    // Ищем файл database.sql в нескольких возможных местах
    $sqlFilePaths = [
    ...
    if (!$sqlFile) {
        $errors[] = "database.sql не найден. Проверьте наличие файла в корне проекта.";
        return $errors;
    }
    ...
    if ($sqlContent === false) {
        $errors[] = "Не удалось прочитать database.sql";
        return $errors;
    }
    
    // Удаляем комментарии и пустые строки
    $lines = array_filter(array_map('trim', explode("\n", $sqlContent)), function($line) {
        return !empty($line) && !preg_match('/^--/', $line);
    });
    
    // Объединяем в одну строку и разбиваем по точкам с запятой
    $sql = implode("\n", $lines);
    ...
    foreach ($statements as $statement) {
        ...
        try {
            ...
            // Некоторые ошибки могут быть некритичными (например, дублирование)
            $errorMsg = $e->getMessage();
            // Игнорируем предупреждения о том, что таблица уже существует
